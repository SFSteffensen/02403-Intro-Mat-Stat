...
#block(width: 100%)[
  ...
  #table(
    ...
    table.header(
      [*What you're given / asked*], [*Statistic (method)*],
      [*Reference dist. \& df*], [*Tail / table*],
    ),
    ...
    [Fixed $n$ trials, count of successes], [Binomial (2.20)], $B(n,p)$, [by hand],
    [Draws *without* replacement], [Hypergeometric (2.24)], $H(n,a,N)$, [by hand],
    [Count of events per interval], [Poisson (2.27)], $"Po"(lambda)$, [by hand],
    [Waiting time between events], [Exponential (2.48)], $"Exp"(lambda)$, [$F(x)=1-e^(-lambda x)$],
  )
]
...
#block(width: 100%)[
  ...
  #table(
    ...
    [CLT — mean treated as normal], [$n >= 30$ (any population shape)],
    [$t$-dist $approx$ normal (replace $t$ by $z$)], [$n >= 30$],
...
#st(
  ...
  $ q_p = cases(
      (x_(n p) + x_(n p + 1))/2 & "for " p n " integer",
      x_(ceil(n p)) & "for " p n " non-integer"
    ) $,
...
#book2py(
  ...
  [$x$], [`k` (observed number of successes, out of $n$)],
)
...
#book2py(
  ...
  [$n$], [`N` (total number of draws)],
  [$x$], [`k` (observed number of successes)],
)
...
$ f(x) = cases(
    1/(x sqrt(2 pi beta^2)) exp(- (ln(x) - alpha)^2/(2 beta^2)) & x > 0,
    0 & x <= 0
  ) $
...
#book2py(
  [$alpha$ (mean of $ln(X)$)], [— (used via `scale`)],
  [$beta$ (std of $ln(X)$)], [`s`],
  [— (axis shift)], [`loc`],
  [$exp(alpha)$], [`scale`],
  [$x$], [`x` (observed value)],
...
#book2py(
  ...
  [$x$], [`x` (waiting time between events)],
)
...
#book2py(
  [$nu_1$], [`dfn` (numerator df)],
  [$nu_2$], [`dfd` (denominator df)],
  [$x$], [`x` (observed value)],
)
...
#st(
  ...
  $ f(x; alpha, beta) = cases(
      0 & x < alpha,
      1/(beta - alpha) & x in [alpha, beta],
      0 & x > beta
    ) \
    F(x; alpha, beta) = cases(
      0 & x < alpha,
      (x - alpha)/(beta - alpha) & x in [alpha, beta],
      1 & x > beta
    ) $,
...
#st(
  ...
  [#cmd("D = pd.DataFrame({'x': x, 'y': y})\nlinfit = smf.ols(formula='y ~ x', data=D).fit()\nprint(linfit.summary(slim=True))") \ Row `Intercept` $-> beta_0$, row `x` $-> beta_1$.],
...
#st(
  ...
  [#cmd("Dn = pd.DataFrame({'x1': x1n, 'x2': x2n})\ntab = linfit.get_prediction(Dn).summary_frame(alpha=0.05)\nci = tab[['mean_ci_lower','mean_ci_upper']]\npi = tab[['obs_ci_lower','obs_ci_upper']]")],
  [Not available.],
...
#block(width: 100%)[
  ...
  #table(
    ...
    [`coef` (row $x_j$)], $hat(beta)_j$, [LS estimate of slope $beta_j$; intercept row $-> hat(beta)_0$],
    [`std err`], $hat(sigma)_(hat(beta)_j)$, [$hat(sigma) sqrt([(bold(X)^T bold(X))^(-1)]_(j j))$],
    [`t`], $t_("obs")$, [$hat(beta)_j \/ hat(sigma)_(hat(beta)_j)$ (tests $H_0: beta_j = 0$)],
    [`P>|t|`], $p$, [two-sided $p$-value, $2 P(T > |t_("obs")|)$, df $= n-p$],
    [`[0.025 0.975]`], [95% CI], [$hat(beta)_j plus.minus t_(0.975)(n-p) dot hat(sigma)_(hat(beta)_j)$],
    [`R-squared`], $r^2$, [$1 - "SSE"\/"SST"$],
    [`F-statistic`], $F_("obs")$, [overall test $H_0:$ all slopes $=0$; in SLR, $F = t_(beta_1)^2$],
    [`Prob (F-statistic)`], $p$, [$p$-value of the overall $F$-test],
    [`No. Observations`], $n$, [],
    [`Df Residuals`], $n - p$, [denominator df for $t$ and $F$],
    [`Df Model`], $p - 1$, [number of slopes (excl. intercept)],
  )
]
...
#block(width: 100%)[
  ...
  #table(
    ...
    table.header(
      ...
      [*Mean sum of squares*], [*Test statistic $F$*], [*$p$-value*],
    ),
    [Treatment], $k-1$, [SS(Tr)],
      $"MS(Tr)" = ("SS(Tr)")/(k-1)$, $F_("obs") = ("MS(Tr)")/("MSE")$,
      $P(F > F_("obs"))$,
...
#block(width: 100%)[
  ...
  #table(
    ...
    table.header(
      [*Source of variation*], [*df*], [*Sums of squares*],
      [*Mean sums of squares*], [*Test statistic*], [*$p$-value*],
    ),
    [Treatment], $k-1$, [SS(Tr)],
      $"MS(Tr)" = ("SS(Tr)")/(k-1)$, $F_("Tr") = ("MS(Tr)")/("MSE")$,
      $P(F > F_("Tr"))$,
    [Block], $l-1$, [SS(Bl)],
      $"MS(Bl)" = ("SS(Bl)")/(l-1)$, $F_("Bl") = ("MS(Bl)")/("MSE")$,
      $P(F > F_("Bl"))$,
    [Residual], $(l-1)(k-1)$, [SSE], $"MSE" = ("SSE")/((k-1)(l-1))$, [], [],
    [Total], $n-1$, [SST], [], [], [],
  )
]
...
+ Compute the test statistic (the $-0$ makes the null hypothesis explicit):
  $ z_("obs") = frac((hat(p)_1 - hat(p)_2) - 0,
